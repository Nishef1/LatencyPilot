...
def apply_implementation_fixes() -> None:
...
def apply_consolidation_fixes() -> None:
    ...
    # single permanent MSTest entrypoint through reflection. CA1822 no longer
    # recognizes them as test callbacks once [TestMethod] is replaced, so scope
    # the suppression only to files containing the reflection marker.
    tests_dir = ROOT / "tests/LatencyPilot.CriticalTests"
    touched = 0
    for path in tests_dir.glob("*.cs"):
        text = path.read_text(encoding="utf-8")
        if "[AuditCase]" not in text:
            continue
        marker = "#pragma warning disable CA1822 // AuditCase methods are reflection-invoked by ConsolidatedCriticalTests.\n"
        if marker not in text:
            path.write_text(marker + text, encoding="utf-8", newline="\n")
        touched += 1
    if touched == 0:
        raise RuntimeError("No consolidated AuditCase files were found for analyzer scoping.")


if __name__ == "__main__":
    if len(sys.argv) == 1:
        apply_implementation_fixes()
    elif len(sys.argv) == 2 and sys.argv[1] == "consolidate":
        apply_consolidation_fixes()
    else:
        raise SystemExit("usage: audit-closure-task6-fix.py [consolidate]")
