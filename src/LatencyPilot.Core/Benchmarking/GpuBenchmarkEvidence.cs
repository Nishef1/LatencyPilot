    ...
    IReadOnlyList<double>? FramePeriodMilliseconds = null)
{
    ...
    // identity is versioned independently. v6 adds a verified in-place GPU
    // restart under the exact Original policy before scored Full/Custom evidence
    // so the initial Original estimate and paired controls share the same
    // post-restart renderer regime. Historical methods are never reinterpreted.
    public const string SchemaId = "latencypilot-gpu-benchmark-v1";
    public const string MethodIdValue = "gpu-affinity-benchmark-v6";
}
