...
internal sealed class BenchmarkWorkload
{
    ...
    private readonly BenchmarkProtocolWriter protocol;
    private readonly IReadOnlyList<LogicalProcessorId> workerMap;

    internal BenchmarkWorkload(BenchmarkOptions options, BenchmarkProtocolWriter protocol, IReadOnlyList<LogicalProcessorId> workerMap)
    { this.options = options; this.protocol = protocol; this.workerMap = workerMap; }
    ...
    internal Task<FrozenBenchmarkWorkload> CalibrateAsync(D3D12BenchmarkRenderer renderer, CancellationToken cancellationToken = default)
    {
        ...
        var cpuSum = 0d; var gpuSum = 0d; var intervalFrames = 0; var lastProgress = TimeSpan.Zero;
        while (started.Elapsed < WarmupDuration)
        {
            cancellationToken.ThrowIfCancellationRequested();
            var frame = renderer.RenderFrame(simulationIterations, commandBatches);
            if (!double.IsFinite(frame.CpuRecordingMilliseconds) || frame.CpuRecordingMilliseconds <= 0 || !double.IsFinite(frame.GpuWorkMilliseconds) || frame.GpuWorkMilliseconds < 0)
                throw new InvalidDataException("Benchmark calibration produced invalid frame timing.");
            cpuSum += frame.CpuRecordingMilliseconds; gpuSum += frame.GpuWorkMilliseconds; intervalFrames++;
            if (intervalStarted.Elapsed >= TimeSpan.FromSeconds(1) && intervalFrames > 0)
            {
                ...
                cpuSum = 0; gpuSum = 0; intervalFrames = 0; intervalStarted.Restart();
            }
            if (started.Elapsed - lastProgress >= ProgressInterval)
            {
                lastProgress = started.Elapsed;
                protocol.WriteProgress(new BenchmarkProgress(BenchmarkProtocolWriter.ProgressSchema, options.SessionId, "calibrating",
        ...
        return Task.FromResult(new FrozenBenchmarkWorkload(commandBatches, simulationIterations, workerMap.ToArray(), options.Seed, options.Width, options.Height));
    }
    ...
    internal Task<BenchmarkTrialArtifact> RunTrialAsync(D3D12BenchmarkRenderer renderer, FrozenBenchmarkWorkload workload, CancellationToken cancellationToken = default)
    {
        ...
        while (stopwatch.Elapsed < options.Duration)
        {
            ...
            if (stopwatch.Elapsed - lastProgress >= ProgressInterval)
            {
                ...
                protocol.WriteProgress(new BenchmarkProgress(BenchmarkProtocolWriter.ProgressSchema, options.SessionId, "measuring",
        ...
        if (frames.Count == 0) throw new InvalidDataException("Benchmark trial completed without any frame evidence.");
        return Task.FromResult(new BenchmarkTrialArtifact(BenchmarkProtocolWriter.ArtifactSchema, options.SessionId, startedAtUtc, DateTimeOffset.UtcNow,
            renderer.AdapterName, renderer.PresentMode, renderer.TimestampFrequency, workload, frames, renderer.CaptureWorkerChecksums()));
    }
