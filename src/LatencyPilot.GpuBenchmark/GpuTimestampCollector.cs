...
internal sealed class GpuTimestampCollector : IDisposable
{
    ...
    internal GpuTimestampCollector(ID3D12Device device, ID3D12CommandQueue queue)
    {
        ...
        if (frequency == 0) throw new InvalidOperationException("D3D12 command queue reported a zero timestamp frequency.");
        queryHeap = device.CreateQueryHeap(new QueryHeapDescription(QueryHeapType.Timestamp, 2));
        readback = device.CreateCommittedResource(HeapType.Readback, ResourceDescription.Buffer(2 * sizeof(ulong)), ResourceStates.CopyDestination);
    }

    internal ulong Frequency => frequency;
    internal void RecordBegin(ID3D12GraphicsCommandList commandList) => commandList.EndQuery(queryHeap, QueryType.Timestamp, 0);
    ...
    internal double ReadElapsedMilliseconds()
    {
        ...
        if (timestamps[1] < timestamps[0]) throw new InvalidDataException("D3D12 GPU timestamps moved backwards.");
        return (timestamps[1] - timestamps[0]) * 1000d / frequency;
    }

    public void Dispose() { readback.Dispose(); queryHeap.Dispose(); }
}
