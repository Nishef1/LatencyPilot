...
if (!parsed.Success) { Console.Error.WriteLine(parsed.Error); return 2; }
var options = parsed.Options!;
var protocol = new BenchmarkProtocolWriter(Console.Out);
...
try
{
    ...
    var workerMap = topology.Cores.Take(options.WorkerCount)
        .Select(static core => core.LogicalProcessors.OrderBy(static processor => processor.Group).ThenBy(static processor => processor.Number).First())
        .ToArray();
    protocol.WriteProgress(new BenchmarkProgress(BenchmarkProtocolWriter.ProgressSchema, options.SessionId, "initializing", 0d,
    ...
    var benchmark = new BenchmarkWorkload(options, protocol, workerMap);
    var frozen = await benchmark.CalibrateAsync(renderer);
    protocol.WriteProgress(new BenchmarkProgress(BenchmarkProtocolWriter.ProgressSchema, options.SessionId, "calibrated", 0d,
    ...
    protocol.WriteProgress(new BenchmarkProgress(BenchmarkProtocolWriter.ProgressSchema, options.SessionId, "complete", 1d,
        $"Benchmark artifact written to {options.OutputPath}."));
    return 0;
}
catch (OperationCanceledException exception)
{
    protocol.WriteFailure(new BenchmarkFailure(BenchmarkProtocolWriter.FailureSchema, options.SessionId, "cancelled", exception.Message));
    return 3;
}
catch (Exception exception)
{
    protocol.WriteFailure(new BenchmarkFailure(BenchmarkProtocolWriter.FailureSchema, options.SessionId, "failed", exception.Message));
    return 1;
}
