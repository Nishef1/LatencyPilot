...
The non-elevated App may inspect stored interrupt policy and translated allocated interrupt resources for latency-sensitive present devices. A development-only one-shot elevated helper permits manual CPU selection only for the already-bounded GPU and USBXHCI affinity targets, plus one bounded PCI `HDAudBus` MSI enable/restore path. It reuses the durable mutation journal, target restart/reboot handling and exact rollback substrate; it does not expose an arbitrary registry writer.

For GPU/xHCI affinity, a manual change first requires translated interrupt-resource affinity to stay entirely inside the requested group-0 mask. GPU Keep additionally requires a clean ETW capture with **direct display-driver** target-only ISR execution; shared `dxgkrnl` fallback is diagnostic only. xHCI uses a controller-specific `USBXHCI` module verifier and fails closed when more than one present controller shares that driver service because the shared module stream cannot be attributed safely to one controller.

For manual HDAudio MSI, applicability requires a present PCI `HDAudBus` target and an existing `MSISupported` DWORD of 0 or 1. LatencyPilot changes only `MSISupported`, never `MessageNumberLimit`, then requires active allocated resources marked `CM_RESOURCE_INTERRUPT_MESSAGE` after restart before Keep. Any failed verification rolls back exact Original or remains recovery-required. Reboot-pending xHCI/MSI work resumes only through the matching journaled experiment. Other device categories remain read-only.
