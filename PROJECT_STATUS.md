...
2. **Evidence:** hosted `Tests` are green on multi-mask source revision `52ee192da8503ec1cb4a3dbcf6b0b023c9f4d443` (workflow run 1638). This proves compilation/analyzer/critical contracts for mask parsing, journaling and UI/source safety checks, but not physical hardware behavior or visual quality. Microsoft documents `AssignmentSetOverride` / `TargetedProcessors` as a KAFFINITY set rather than a single processor bit. This ledger-only update must also be exact-head green.
